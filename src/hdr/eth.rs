...
pub mod eth_type {
    pub const ARP: usize = 0x0800;
    #[allow(non_upper_case_globals)]
    pub const IPv4: usize = 0x0806;
}
...
impl EthHdr {
    ...
    pub fn from<T: ToString>(src_addr: T, dst_addr: T, eth_type: u16) -> Result<Self, PaError> {
        ...
        if src_addr.len() != 6 || dst_addr.len() != 6 {
            ...
            for i in 0..6 {
                src_hw_addr[i] = u8::from_str_radix(src_addr[i], 16).unwrap_or_else(|err| {
                    panic!("{:?}", PaError::new(err.to_string(), ErrorType::ParseError))
                });
                dst_hw_addr[i] = u8::from_str_radix(dst_addr[i], 16).unwrap_or_else(|err| {
                    panic!("{:?}", PaError::new(err.to_string(), ErrorType::ParseError))
                });
            }
    ...
    pub fn encapsulate(&self, data: impl Hdr) -> Result<Vec<u8>, PaError> {
